...
#PATH
#PATH_Input <- "/home/pezoldt/NAS2/pezoldt/Analysis/34_35_Species_Mixing"
#PATH_mac
...
for(i in 1:length(sample_Ids)){
  ...
  #t_UMI_i <- t_UMI_i[,c("Gene_id","Gene_name","Human","Mouse")]
  #New column names
...
##### Igepal
#Select functioning datasets for species mixing plot
...
ggplot(t_pivot_half, aes(x=read_n_map, y=UMI_human)) + 
  geom_point(aes(col=cell_number),size= 4) +   # draw points
  xlim(c(0, 300000)) + 
  ylim(c(0, 200000)) +   # draw smoothing line
  scale_color_manual(values = c("black","red","orange","deepskyblue","green")) +
  labs(y="[n] UMIs human", 
       x="[n] reads mapped", 
       title="Species Mixing, LiD_v4")
...
threshed_mean <- subset(t_pivot_half, read_n_seq > 100000)
mean(threshed_mean$UMI_human)
mean(threshed_mean$UMI_mouse)

mean(threshed_mean$UMI_mouse) / mean(threshed_mean$UMI_human)*100
